 ...
 * @constructor
 */
require( 'abstract' ).createModel( function Stream( Stream, prototype ) {

  // Expose
  module.exports = this;

  // Inherit Stream functionality
  Stream.use( require( '_stream_readable' ).prototype );
  Stream.use( require( '_stream_writable' ).prototype );
  Stream.use( require( '_stream_duplex' ).prototype );

  // console.log( require( 'util' ).inspect( Stream.__proto__, { showHidden: true, colors: true } ) )
  ...
  // Constructor
  Stream.properties({
    defaults: function defaults( defaults ) {
      return Stream.set( 'defaults', defaults );
    },
    eventify: function eventify( target, namespace, options ) {

      // Enable EventChannel
      Stream.extend( target, namespace, options );
  ...
  // Instance
  Stream.properties( prototype, {

    // EventEmitter Methods
    once: function once( event, fn ) {
      // Stream.logger.debug( arguments.callee.name, arguments[0], typeof arguments[1] );
      return this.many( event, 1, fn );
    },
    many: function many( event, ttl, fn ) {
      // Stream.logger.debug( arguments.callee.name, arguments[0], typeof arguments[1], typeof arguments[2] );
    ...
    emit: function emit() {
      // Stream.logger.debug( arguments.callee.name, arguments[0] );
      ...
      if( typeof handler === 'function' ) {
        //// Stream.logger.debug( '%s() handler %s IS a function', arguments.callee.name, type );
        this.event = type;
        ...
        //// Stream.logger.debug( '%s() handler %s is not a function', arguments.callee.name, type );
        var l = arguments.length;
        var args = new Array( l - 1 );
        for( var i = 1; i < l; i++ ) {
          args[i - 1] = arguments[i];
    ...
    on: function on( type, listener ) {
      ...
      if( typeof listener !== 'function' ) {
        // Stream.logger.error( this.constructor.name, ':', arguments.callee.name, ' - callback must be typeof function, not', typeof listener, 'as provided.' )
        if( this.settings.throw ) { throw new Error( 'on only accepts instances of Function' ); } else { return this; }
      }

      this._events || Stream.call( this );
    ...
    // Stream.logger.debug( arguments.callee.name, handlers, type, typeof tree, i );
    ...
    unpipe: require( 'stream' ).prototype.unpipe,
  ...
  // Instantiate
  Stream.defineInstance( function create( options ) {
    var Instance = this;

    Instance.set( 'emitter', options );
